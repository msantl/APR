...
Matrica Matrica::operator! (void) {
    if (_rows != _cols) throw "Inverz matrice, pogresne dimenzije";

    Matrica result(_rows, _cols);

    try {
        /* A * A' = I */
        Matrica yi(_rows, 1), xi(_rows, 1), b(_rows, 1);
        ...
        for (int i = 0; i < _rows; ++i) {
            ...
            yi = A.forward_substitution(b);
            xi = A.backward_substitution(yi);
